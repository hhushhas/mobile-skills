...
Use this reference for native auth setup, Sign in with Apple, Google Sign-In, Firebase Auth, OAuth provider config, Universal Links, Android App Links, custom schemes, and reviewer access.
...
3. Extract Android SHA-1/SHA-256 fingerprints from debug/release keystores and, when available, Play App Signing.
...
7. Create reviewer/demo account instructions and update submission notes/packet refs.

## Automatable

Apple:

- Create/locate Bundle ID with App Store Connect API.
- Enable supported capabilities such as `APPLE_ID_AUTH` and `ASSOCIATED_DOMAINS`.
- Patch entitlements and `Info.plist` URL schemes.
- Regenerate/download provisioning profiles through App Store Connect API or fastlane `sigh`/`match`.
...
- Initial App Store Connect API key creation/download.
...
- Play App Signing SHA fingerprints before the app has Play Console access or an uploaded artifact.
...
- Play App Signing certificate SHA can differ from local upload/debug keystores.
...
- App Store Connect API Bundle IDs: https://developer.apple.com/documentation/appstoreconnectapi/bundle-ids
